...
class Historico extends Turma {
  ...
  double media(Aluno aluno) {
    List<double>? notasAluno = notas[aluno];
    if (notasAluno == null || notasAluno.isEmpty) return 0.0;

    double soma = notasAluno.reduce((a, b) => a + b);
...
void main() {
  test('Testar matrícula de alunos', () {
    ...
    // Adicionando notas para testar aprovação
    historico1.notas[aluno1]?.addAll([7.0, 6.0]);
    expect(historico1.media(aluno1), closeTo(6.5, 0.01));
